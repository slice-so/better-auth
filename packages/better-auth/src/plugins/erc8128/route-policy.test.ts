...
describe("resolveRoutePolicy — multi-method keys", () => {
	...
	it("matches comma-separated methods to a single path", () => {
		...
		const post = resolveRoutePolicy(routePolicy, fakeRequest("POST", "/api/orders"));
		expect(post.requireAuth).toBe(true);

		const get = resolveRoutePolicy(routePolicy, fakeRequest("GET", "/api/orders"));
		expect(get.requireAuth).toBe(true);

		const put = resolveRoutePolicy(routePolicy, fakeRequest("PUT", "/api/orders"));
	...
	it("does not match unlisted methods", () => {
		...
		const del = resolveRoutePolicy(routePolicy, fakeRequest("DELETE", "/api/orders"));
	...
	it("works with wildcards in the path", () => {
		...
		const get = resolveRoutePolicy(routePolicy, fakeRequest("GET", "/api/items/123"));
		expect(get.requireAuth).toBe(true);

		const post = resolveRoutePolicy(routePolicy, fakeRequest("POST", "/api/items/456"));
		expect(post.requireAuth).toBe(true);

		const put = resolveRoutePolicy(routePolicy, fakeRequest("PUT", "/api/items/789"));
	...
	it("supports false to skip verification for multiple methods", () => {
		...
		const get = resolveRoutePolicy(routePolicy, fakeRequest("GET", "/api/public"));
		expect(get.skipVerification).toBe(true);

		const post = resolveRoutePolicy(routePolicy, fakeRequest("POST", "/api/public"));
